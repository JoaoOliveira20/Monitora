...
import type { HostTarget, HttpTarget, LogTarget } from "../src/config/schema.js";
import { evaluateAlert, formatDuration } from "../src/monitoring/alert-policy.js";
import type { CheckResult, HostCheckMetadata, LogCheckMetadata, MonitorState, StateTransition } from "../src/types/index.js";
...
function buildHostResult(overrides: Partial<CheckResult<HostCheckMetadata>> & { checkedAt: Date }): CheckResult<HostCheckMetadata> {
  return {
    targetId: "target-1",
    success: true,
    durationMs: 10,
...
test("evaluateAlert generates a HOST_THRESHOLD event immediately on UP -> DOWN transition, ignoring cooldown", () => {
  const target = buildHostTarget({ cooldownSeconds: 900 });
  const state = buildState({ status: "DOWN", lastAlertAt: at(0) });
  const transition: StateTransition = {
    targetId: "target-1",
    previousStatus: "UP",
  ...
  assert.equal(event?.type, "HOST_THRESHOLD");
  assert.deepEqual(event?.metadata, { cpuPercent: 94.2, memoryPercent: 68.1, diskPercent: 72.4 });
  assert.match(event?.message ?? "", /CPU: 94\.2%/);
  assert.match(event?.message ?? "", /Memory: 68\.1%/);
  assert.match(event?.message ?? "", /Disk: 72\.4%/);
});

test("evaluateAlert generates a RECOVERED event on DOWN -> UP transition for a host target", () => {
  ...
  const result = buildHostResult({ success: true, checkedAt: at(900) });
...
test("evaluateAlert does not send a host reminder before cooldownSeconds has elapsed", () => {
  const target = buildHostTarget({ cooldownSeconds: 900 });
  const state = buildState({ status: "DOWN", lastAlertAt: at(0) });
  const result = buildHostResult({ success: false, checkedAt: at(500), metadata: { cpuPercent: 95 } });
...
test("evaluateAlert sends a host threshold reminder once cooldownSeconds has elapsed for a persistent condition", () => {
  const target = buildHostTarget({ cooldownSeconds: 900 });
  const state = buildState({ status: "DOWN", lastAlertAt: at(0) });
  const result = buildHostResult({ success: false, checkedAt: at(900), metadata: { cpuPercent: 96 } });
